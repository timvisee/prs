...
use thiserror::Error;
use totp_rs::{Builder, Secret, Totp as RawTotp};
...
///
/// Uses RFC6238 defaults, see: https://tools.ietf.org/html/rfc6238
fn parse_encoded(plaintext: &Plaintext) -> Option<Totp> {
    ...
    let secret = Secret::try_from_base32(plaintext).unwrap();

    // Parse RFC6238 TOTP (with looser requirements)
    Some(
        Builder::new()
            .with_secret(secret)
            // account name is required to later generate a URL, we don't have one here
            .with_account_name("-")
            .build_noncompliant()
            .into(),
    )
...
impl Totp {
    ...
    pub fn from_url(url: &str) -> Result<Self> {
        RawTotp::from_url_unchecked(url)
            .map(|t| t.into())
            .map_err(|e| Err::Url(e).into())
    }

    /// Generate a token from the current system time.
    pub fn generate_current(&self) -> Plaintext {
        self.totp.generate_current().to_string().into()
    }

    /// Generate an URL for this TOTP secret.
    pub fn generate_url(&self) -> Result<Plaintext> {
        self.totp
            .to_url()
            .map(|s| s.into())
            .map_err(|e| Err::GenerateUrl(e).into())
    }
...
impl From<RawTotp> for Totp {
    fn from(totp: RawTotp) -> Self {
        Self { totp }
    }
}
...
pub enum Err {
    ...
    Url(#[source] totp_rs::TotpError),

    #[error("failed to generate TOTP url")]
    GenerateUrl(#[source] totp_rs::TotpError),
